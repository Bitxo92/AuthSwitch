...
from .._session import sync_session_manager
...
class UsuarioSyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``Usuario``.
    ...
    Attributes:
        ...
        _df_validator (UsuarioDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[UsuarioRead]:
    ...
    Métodos de escritura:
        create(usuario, *operative_fields, session) → UsuarioRead:
    ...
    Métodos de upsert:
        upsert(usuario, match_fields, session) → UsuarioRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(UsuarioCreate(...))
        dao.update(id=nuevo.id, updated_values=UsuarioUpdateValues(...))
        dao.delete(id=nuevo.id)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(UsuarioCreate(...), session=session)
            b = dao.create(UsuarioCreate(...), session=session)
    ...
    @classmethod
    def _on_create_before(cls, new_usuario: Usuario, included: List[str], session: Session):
        from ..post.dao_sync import PostSyncDAO

        # Cascade: Post triggers (one-to-many)
        if 'posts' in included:
            for _child in new_usuario.posts:
                PostSyncDAO._on_create_before(_child, included, session)

    @classmethod
    def _on_create_after(cls, new_usuario: Usuario, included: List[str], session: Session):
        from ..post.dao_sync import PostSyncDAO

        # Cascade: Post triggers (one-to-many)
        if 'posts' in included:
            for _child in new_usuario.posts:
                PostSyncDAO._on_create_after(_child, included, session)

    @classmethod
    def _on_update_before(cls, new_usuario: Usuario, old_usuario: UsuarioRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_update_after(cls, new_usuario: Usuario, old_usuario: UsuarioRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_delete_before(cls, old_usuario: UsuarioRead, session: Session):
        pass

    @classmethod
    def _on_delete_after(cls, old_usuario: UsuarioRead, session: Session):
        pass
    ...
    def find(
        ...
        """
        _log_tx(session, f"[public] 🔍 Buscando Usuario:")
        ...
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, Usuario, rls)
        ...
        def execute_query(session: Session) -> Optional[UsuarioRead]:
            ...
            if instance:
                _log_commit(session, f"[public] ✅ Usuario encontrado exitosamente")
                ...
                _log_commit(session, f"[public] 📭 Usuario no encontrado")
                return None
    ...
    def find_many(
        ...
        """
        _log_tx(session, f"[public] 🔍 Buscando múltiples Usuario:")
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(Usuario, column_name):
                    ...
                    logger.warning(f"[public] ⚠️ Columna '{column_name}' no existe en modelo Usuario, ignorando en order_by")
        ...
        def execute_query(session: Session) -> List[UsuarioRead]:
            ...
            _log_commit(session, f"[public] ✅ Encontrados {len(instances)} registros Usuario")
    ...
    def count(
        ...
        """
        _log_tx(session, f"[public] 🔢 Contando registros Usuario con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Conteo Usuario completado: {count_result} registros")
        return count_result
    ...
    def exists(
        ...
        """
        _log_tx(session, f"[public] ❓ Verificando existencia de registros Usuario")
        records = cls.count(
            name=name,
            in_name=in_name,
            email=email,
        ...
        _log_commit(session, f"[public] ✅ Verificación Usuario completada: {'existe' if exists_result else 'no existe'}")
    ...
    @classmethod
    def _optimize_dataframe_dtypes(cls, df: DataFrame) -> DataFrame:
        """
    ...
    def from_dataframe(
        ...
        Example:
            ...
            df = pd.DataFrame({
                ...
                'pwd': ['valor1', 'valor2', 'valor3'],
                'email': ['valor1', 'valor2', 'valor3'],
                'last_post_date': [None, None, None]
            })
        ...
        if mode == 'create':
            return cls.create_many([UsuarioCreate.from_dict(record) for record in records_data])
        else:  # upsert
            return cls.upsert_many([UsuarioCreate.from_dict(record) for record in records_data], match_fields=match_fields)
    ...
    def create(
        ...
        """
        _log_tx(session, f"[public] 🆕 Creando nuevo Usuario")
        new_instance = usuario.to_instance()

        def _execute_create(session: Session):
            included = get_included_relations(new_instance, usuario)
            cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            session.flush()
            cls._on_create_after(new_instance, included, session)
            return UsuarioRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[public] ✅ Usuario creado exitosamente con id={data.id}")
        return data
    ...
    def create_many(
        ...
        """
        _log_tx(session, f"[public] 🔢 Creando {len(records)} registros Usuario")

        def _execute_create_many(session: Session) -> int:
            instances = []
            for record in records:
                instance = record.to_instance()
                included = get_included_relations(instance, record)
                cls._on_create_before(instance, included, session)
                instances.append(instance)
            session.add_all(instances)
            session.flush()
            for instance in instances:
                included = get_included_relations(instance, record)
                cls._on_create_after(instance, included, session)
        ...
        _log_commit(session, f"[public] ✅ {result} registros Usuario creados exitosamente")
    ...
    def update(
        ...
        _log_tx(session, f"[public] 🔄 Actualizando Usuario:")
        _log_tx(session, f"[public]     id={id}")
        ...
        def _execute_update(session: Session) -> int:
            from ..post.dao_sync import PostSyncDAO
            result = session.execute(query)
            record = result.scalar_one_or_none()
            if record is None:
                _log_tx(session, f"[public] ⚠️ No se encontró ningún registro Usuario para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = UsuarioRead.from_instance(record)
            for key, value in update_data.items():
                setattr(record, key, value)
            new_instance = record

            cls._on_update_before(new_instance, old_dto, update_data, session)
            session.flush()
            cls._on_update_after(new_instance, old_dto, update_data, session)

            if updated_values.posts is not None:
                _nested = updated_values.posts
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.author_id = id
                        PostSyncDAO.create(_create_dto, session=session)
                if _nested.update:
        ...
        if result > 0:
            _log_commit(session, f"[public] ✅ 1 registros Usuario actualizados exitosamente")
        else:
            _log_tx(session, f"[public] ⚠️ No se encontró ningún registro Usuario para actualizar con los identificadores proporcionados.")
    ...
    def update_many(
        ...
        """
        _log_tx(session, f"[public] 🔄 Actualizando múltiples Usuario con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[public] ✅ {result.rowcount} registros Usuario actualizados masivamente exitosamente")
    ...
    def delete(
        ...
        """
        _log_tx(session, f"[public] 🗑️ Eliminando Usuario:")
        _log_tx(session, f"[public]    id={id}")

        def _execute_delete(session: Session) -> int:
            fetch_query = select(Usuario)
            fetch_query = fetch_query.where(Usuario.id == id)
            ...
            cls._on_delete_before(old_dto, session)

            del_query = delete(Usuario)
            del_query = del_query.where(Usuario.id == id)
            result = session.execute(del_query)

            cls._on_delete_after(old_dto, session)
        ...
        _log_commit(session, f"[public] ✅ {deleted} registros Usuario eliminados exitosamente")
    ...
    def delete_many(
        ...
        def execute_query(session: Session) -> int:
            ...
            _log_tx(session, f"[public] 🗑️  Eliminando múltiples Usuario con filtros: {filters}")
        ...
        _log_commit(session, f"[public] ✅ {total_deleted} registros Usuario eliminados masivamente exitosamente")

        return total_deleted

    @classmethod
    def _build_native_upsert_stmt(cls, dialect_name: str, values: Union[Dict[str, Any], List[Dict[str, Any]]], match_fields: List[str], update_columns: List[str]):
        """
        ...
        if dialect_name in ('postgresql', 'sqlite'):
            ...
            stmt = dialect_insert(Usuario).values(values)
            if update_columns:
            ...
            stmt = mysql_insert(Usuario).values(values)
            if update_columns:
    ...
    def upsert(
        ...
        Args:
            usuario: Datos del registro a insertar o actualizar
        ...
        """
        _log_tx(session, f"[public] 🔄 Upsert Usuario con campos de coincidencia: {match_fields}, datos: {usuario.to_dict()}")

        record_dict = usuario.to_dict()
        ...
        for field in match_fields:
            if field not in record_dict:
                raise ValueError(f"El campo '{field}' debe estar presente en 'usuario' para upsert.")
        ...
        def execute_query(session: Session) -> UsuarioRead:
            # SELECT para determinar si es create o update
            query = select(Usuario)
            for field in match_fields:
                query = query.where(getattr(Usuario, field) == record_dict[field])
            existing = session.execute(query).scalars().first()

            if existing:
                update_values = UsuarioUpdateValues(**{k: record_dict[k] for k in update_columns if k in record_dict})
                cls.update(
                    id=existing.id,
                ...
                return UsuarioRead.from_instance(result)
            else:
                return cls.create(usuario, session=session)

        if session is not None:
            result = execute_query(session)
            _log_commit(session, f"[public] ✅ Upsert Usuario completado exitosamente")
            return result
        else:
            with sync_session_manager.get_session() as session:
                result = execute_query(session)
    ...
    def upsert_many(
        ...
        _log_tx(session, f"[public] 🔄 Upsert masivo Usuario con {len(records)} registros, campos de coincidencia: {match_fields}")
                ...
                if stmt is not None:
                    ...
                    for record in batch:
                        if len(match_fields) == 1:
                            ...
                            conditions.append(getattr(Usuario, field) == record[field])
                        else:
                            record_conds = [getattr(Usuario, f) == record[f] for f in match_fields]
                            conditions.append(and_(*record_conds))
                    
                    query = select(Usuario).where(or_(*conditions))
                    existing_records = session.execute(query).scalars().all()
                    ...
                    if to_insert:
                        session.execute(Usuario.__table__.insert(), to_insert)
                    
                    # Bulk update de registros existentes
                    for record in to_update:
                        update_stmt = update(Usuario)
                        for field in match_fields:
                            update_stmt = update_stmt.where(getattr(Usuario, field) == record[field])
                        update_values = {k: v for k, v in record.items() if k not in match_fields}
                        if update_values:
                            session.execute(update_stmt.values(**update_values))
        ...
        _log_commit(session, f"[public] ✅ Upsert masivo Usuario completado: {result} registros procesados")
    ...
    def sum(
        ...
        """
        _log_tx(session, f"[public] 🧮 Sumando campos {agg_fields} de registros Usuario con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(Usuario, field):
                ...
                error_msg = f"Campo '{field}' no existe en modelo Usuario"
        ...
        _log_commit(session, f"[public] ✅ Suma Usuario completada: {sum_result}")
    ...
    def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 📊 Calculando media de campos {agg_fields} de registros Usuario con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Media Usuario completada: {mean_result}")
    ...
    def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 🔺 Calculando máximo de campos {agg_fields} de registros Usuario con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Máximo Usuario completado: {max_result}")
    ...
    def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 🔻 Calculando mínimo de campos {agg_fields} de registros Usuario con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Mínimo Usuario completado: {min_result}")
    ...
    def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[public] 🎯 Realizando agregaciones en Usuario: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, Usuario)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(Usuario, expression):
                            column      = getattr(Usuario, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo Usuario")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(Usuario, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo Usuario")
                    continue

                column      = getattr(Usuario, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[public] ✅ Agregaciones Usuario completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
