...
from ..usuario.dtos import *  # trigger cross-table: Usuario
...
class PostSyncDAO:
    ...
    @classmethod
    def _on_create_before(cls, new_post: Post, included: List[str], session: Session):
        ...
        if 'comments' in included:
            for _child in new_post.comments:
                CommentSyncDAO._on_create_before(_child, included, session)

        # Cascade: Usuario triggers (many-to-one)
        if 'author' in included and new_post.author is not None:
            UsuarioSyncDAO._on_create_before(new_post.author, included, session)

    @classmethod
    def _on_create_after(cls, new_post: Post, included: List[str], session: Session):
        from ..usuario.dao_sync import UsuarioSyncDAO
        from ..comment.dao_sync import CommentSyncDAO

        # 🔔 Trigger: modify_user_last_post_date — Trigger que actualiza el campo last_post_date del usuario al crear un nuevo post
        UsuarioSyncDAO.update(new_post.author_id, updated_values=UsuarioUpdateValues(last_post_date=new_post.timestamp), session=session)

        # Cascade: Comment triggers (one-to-many)
        if 'comments' in included:
            for _child in new_post.comments:
                CommentSyncDAO._on_create_after(_child, included, session)

        # Cascade: Usuario triggers (many-to-one)
        if 'author' in included and new_post.author is not None:
            UsuarioSyncDAO._on_create_after(new_post.author, included, session)

    @classmethod
    def _on_update_before(cls, new_post: Post, old_post: PostRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_update_after(cls, new_post: Post, old_post: PostRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_delete_before(cls, old_post: PostRead, session: Session):
        pass

    @classmethod
    def _on_delete_after(cls, old_post: PostRead, session: Session):
        pass
    ...
    def find(
        ...
        # Ejecutar query
        def execute_query(session: Session) -> Optional[PostRead]:
    ...
    def find_many(
        ...
        # Ejecutar query
        def execute_query(session: Session) -> List[PostRead]:
    ...
    def from_dataframe(
        ...
        return cls.create_many([PostCreate.from_dict(record) for record in records_data])
            ...
            if updated_values.comments is not None:
                ...
                if _nested.create:
                    for _create_dto in _nested.create:
                        ...
                        CommentSyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        CommentSyncDAO.update(
                            id=_update_dto.id,
