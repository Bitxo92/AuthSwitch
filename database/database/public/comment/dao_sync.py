...
class CommentSyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``Comment``.
    ...
    Attributes:
        ...
        _df_validator (CommentDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[CommentRead]:
    ...
    Métodos de escritura:
        create(comment, *operative_fields, session) → CommentRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(CommentCreate(...))
        dao.update(id=nuevo.id, updated_values=CommentUpdateValues(...))
        dao.delete(id=nuevo.id)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(CommentCreate(...), session=session)
            b = dao.create(CommentCreate(...), session=session)
    ...
    @classmethod
    def _on_create_before(cls, new_comment: Comment, included: List[str], session: Session):
        from ..post.dao_sync import PostSyncDAO

        # Cascade: Post triggers (many-to-one)
        if 'post' in included and new_comment.post is not None:
            PostSyncDAO._on_create_before(new_comment.post, included, session)

    @classmethod
    def _on_create_after(cls, new_comment: Comment, included: List[str], session: Session):
        from ..post.dao_sync import PostSyncDAO

        # Cascade: Post triggers (many-to-one)
        if 'post' in included and new_comment.post is not None:
            PostSyncDAO._on_create_after(new_comment.post, included, session)

    @classmethod
    def _on_update_before(cls, new_comment: Comment, old_comment: CommentRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_update_after(cls, new_comment: Comment, old_comment: CommentRead, update_data: dict, session: Session):
        pass

    @classmethod
    def _on_delete_before(cls, old_comment: CommentRead, session: Session):
        pass

    @classmethod
    def _on_delete_after(cls, old_comment: CommentRead, session: Session):
        pass
    ...
    def find(
        ...
        """
        _log_tx(session, f"[public] 🔍 Buscando Comment:")
        ...
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, Comment, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(Comment, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        def execute_query(session: Session) -> Optional[CommentRead]:
            result = session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[public] ✅ Comment encontrado exitosamente")
                return CommentRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[public] 📭 Comment no encontrado")
                return None
    ...
    def find_many(
        ...
        - min_post_id: Filtrar por valor mínimo de post_id (incluído)
        - max_post_id: Filtrar por valor máximo de post_id (incluído)
        ...
        """
        _log_tx(session, f"[public] 🔍 Buscando múltiples Comment:")
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(Comment, column_name):
                    column = getattr(Comment, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[public] ⚠️ Columna '{column_name}' no existe en modelo Comment, ignorando en order_by")
        ...
        # Ejecutar query
        def execute_query(session: Session) -> List[CommentRead]:
            results = session.execute(query)
            instances = results.scalars().all()
            
            _log_commit(session, f"[public] ✅ Encontrados {len(instances)} registros Comment")

            return [
                CommentRead.from_instance(
                    instance, 
                    includes=includes, 
                    max_depth=5
                ) 
    ...
    def count(
        ...
        """
        _log_tx(session, f"[public] 🔢 Contando registros Comment con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Conteo Comment completado: {count_result} registros")
        return count_result
    ...
    def exists(
        ...
        """
        _log_tx(session, f"[public] ❓ Verificando existencia de registros Comment")
        records = cls.count(
            content=content,
            in_content=in_content,
            post_id=post_id,
        ...
        _log_commit(session, f"[public] ✅ Verificación Comment completada: {'existe' if exists_result else 'no existe'}")
    ...
    def as_dataframe(
        ...
        Example:
            ...
            df = db_api.comment.as_dataframe()
    ...
    def from_dataframe(
        ...
        return cls.create_many([CommentCreate.from_dict(record) for record in records_data])
    ...
    def create(
        ...
        Args:
            comment: Datos del comment a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[public] 🆕 Creando nuevo Comment")
        new_instance = comment.to_instance()

        def _execute_create(session: Session):
            included = get_included_relations(new_instance, comment)
            cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            session.flush()
            cls._on_create_after(new_instance, included, session)
            return CommentRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[public] ✅ Comment creado exitosamente con id={data.id}")
        return data
    ...
    def create_many(
        cls, 
        records: List[CommentCreate], 
        session: Optional[Session] = None
    ) -> int:
        """
        Crea múltiples registros en la tabla comment.
        
        Args:
            records: Lista de CommentCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[public] 🔢 Creando {len(records)} registros Comment")
        ...
        _log_commit(session, f"[public] ✅ {result} registros Comment creados exitosamente")
    ...
    def update(
        cls, 
        id: int,
        updated_values: CommentUpdateValues,
        session: Optional[Session] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[public] 🔄 Actualizando Comment:")
        _log_tx(session, f"[public]     id={id}")
        ...
        def _execute_update(session: Session) -> int:
            ...
            if record is None:
                _log_tx(session, f"[public] ⚠️ No se encontró ningún registro Comment para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = CommentRead.from_instance(record)
        ...
        if result > 0:
            _log_commit(session, f"[public] ✅ 1 registros Comment actualizados exitosamente")
        else:
            _log_tx(session, f"[public] ⚠️ No se encontró ningún registro Comment para actualizar con los identificadores proporcionados.")
    ...
    def update_many(
        ...
        """
        _log_tx(session, f"[public] 🔄 Actualizando múltiples Comment con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[public] ✅ {result.rowcount} registros Comment actualizados masivamente exitosamente")
    ...
    def delete(
        ...
        """
        _log_tx(session, f"[public] 🗑️ Eliminando Comment:")
        _log_tx(session, f"[public]    id={id}")

        def _execute_delete(session: Session) -> int:
            fetch_query = select(Comment)
            fetch_query = fetch_query.where(Comment.id == id)
            ...
            del_query = delete(Comment)
            del_query = del_query.where(Comment.id == id)
        ...
        _log_commit(session, f"[public] ✅ {deleted} registros Comment eliminados exitosamente")
    ...
    def delete_many(
        ...
        def execute_query(session: Session) -> int:
            ...
            _log_tx(session, f"[public] 🗑️  Eliminando múltiples Comment con filtros: {filters}")
        ...
        _log_commit(session, f"[public] ✅ {total_deleted} registros Comment eliminados masivamente exitosamente")
    ...
    def sum(
        ...
        """
        _log_tx(session, f"[public] 🧮 Sumando campos {agg_fields} de registros Comment con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(Comment, field):
                ...
                error_msg = f"Campo '{field}' no existe en modelo Comment"
        ...
        _log_commit(session, f"[public] ✅ Suma Comment completada: {sum_result}")
    ...
    def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 📊 Calculando media de campos {agg_fields} de registros Comment con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Media Comment completada: {mean_result}")
    ...
    def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 🔺 Calculando máximo de campos {agg_fields} de registros Comment con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Máximo Comment completado: {max_result}")
    ...
    def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[public] 🔻 Calculando mínimo de campos {agg_fields} de registros Comment con filtros aplicados")
        ...
        _log_commit(session, f"[public] ✅ Mínimo Comment completado: {min_result}")
    ...
    def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[public] 🎯 Realizando agregaciones en Comment: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, Comment)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(Comment, expression):
                            column      = getattr(Comment, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo Comment")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(Comment, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo Comment")
                    continue
        ...
        _log_commit(session, f"[public] ✅ Agregaciones Comment completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
