...
from ...database import UsuarioAsyncDAO, UsuarioFilter, AggRow, AggRequest
...
class AggregationPayload(BaseModel):
    ...
    filters: Optional[UsuarioFilter] = None

@usuario_router.post("/agg",
    response_model=APIResponse[List[AggRow]],
    operation_id="public_usuario_aggregate",
    summary=AGG_SUMMARY,
...
async def usuario_aggregate(
    ...
    with username_context(token.preferred_username):
        result = await UsuarioAsyncDAO.agg(
