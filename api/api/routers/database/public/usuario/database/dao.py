...
class UsuarioAsyncDAO:
    ...
    @classmethod
    async def _on_create_before(cls, new_usuario: Usuario, included: List[str], session: AsyncSession):
        from ...post.database.dao import PostAsyncDAO

        # Cascade: Post triggers (one-to-many)
        if 'posts' in included:
            for _child in new_usuario.posts:
                await PostAsyncDAO._on_create_before(_child, included, session)

    @classmethod
    async def _on_create_after(cls, new_usuario: Usuario, included: List[str], session: AsyncSession):
        from ...post.database.dao import PostAsyncDAO

        # Cascade: Post triggers (one-to-many)
        if 'posts' in included:
            for _child in new_usuario.posts:
                await PostAsyncDAO._on_create_after(_child, included, session)

    @classmethod
    async def _on_update_before(cls, new_usuario: Usuario, old_usuario: UsuarioRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_usuario: Usuario, old_usuario: UsuarioRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_usuario: UsuarioRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_usuario: UsuarioRead, session: AsyncSession):
        pass
    ...
    async def from_dataframe(
        ...
        if mode == 'create':
            return await cls.create_many([UsuarioCreate.from_dict(record) for record in records_data])
        else:  # upsert
            return await cls.upsert_many([UsuarioCreate.from_dict(record) for record in records_data], match_fields=match_fields)
            ...
            if updated_values.posts is not None:
                ...
                if _nested.create:
                    for _create_dto in _nested.create:
                        ...
                        await PostAsyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        await PostAsyncDAO.update(
                            id=_update_dto.id,
    ...
    async def upsert(
        ...
        async def execute_query(session: AsyncSession) -> UsuarioRead:
            ...
            if existing:
                ...
                return await cls.create(usuario, session=session)
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            for i in range(0, len(records_data), batch_size):
                ...
                if stmt is not None:
                    ...
                    if to_insert:
                        await session.execute(Usuario.__table__.insert(), to_insert)
