...
from .docs import MIN_RESPONSES, MIN_SUMMARY
from ... import user_stats_router

@user_stats_router.get("/{field}/min",
    response_model=APIResponse[List[AggRow]],
    operation_id="public_user_stats_min_field",
    summary=MIN_SUMMARY,
    responses=MIN_RESPONSES
)
async def user_stats_min_field(
    ...
    ## Parámetros
    - **field** (path): Nombre del campo para encontrar el mínimo.
    ...
    with username_context(token.preferred_username):
        result = await UserStatsAsyncDAO.min(
            agg_fields=[field],
            user_id=user_id,
        ...
        if not result.success:
            return APIResponse.error(
                ...
                message=f"Error al calcular el mínimo de '{field}'"
            )

        return APIResponse.success(
            data=result.rows,
            message=f"Mínimo de '{field}' calculado exitosamente",
