...
from ...database import UserStatsAsyncDAO, UserStatsFilter, AggRow, AggRequest
...
class AggregationPayload(BaseModel):
    ...
    filters: Optional[UserStatsFilter] = None

@user_stats_router.post("/agg",
    response_model=APIResponse[List[AggRow]],
    operation_id="public_user_stats_aggregate",
    summary=AGG_SUMMARY,
    description=AGG_DESCRIPTION,
    responses=AGG_RESPONSES
)
async def user_stats_aggregate(
    ...
    """
    Realiza múltiples operaciones de agregación en una sola consulta sobre la tabla user_stats.
    """
    with username_context(token.preferred_username):
        result = await UserStatsAsyncDAO.agg(
