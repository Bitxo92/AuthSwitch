...
@post_router.get("/count",
    ...
    response_description="Número de registros de post según los filtros aplicados",
    operation_id="public_post_count",
...
async def post_count(
    ...
    """
    Cuenta el número de Posts que coinciden con los filtros.
    """
    with username_context(token.preferred_username):
        result = await PostAsyncDAO.count(
            title=title,
            in_title=in_title,
            content=content,
