...
class CommentAsyncDAO:
    ...
    @classmethod
    async def _on_create_before(cls, new_comment: Comment, included: List[str], session: AsyncSession):
        ...
        if 'post' in included and new_comment.post is not None:
            await PostAsyncDAO._on_create_before(new_comment.post, included, session)

    @classmethod
    async def _on_create_after(cls, new_comment: Comment, included: List[str], session: AsyncSession):
        from ...post.database.dao import PostAsyncDAO

        # Cascade: Post triggers (many-to-one)
        if 'post' in included and new_comment.post is not None:
            await PostAsyncDAO._on_create_after(new_comment.post, included, session)

    @classmethod
    async def _on_update_before(cls, new_comment: Comment, old_comment: CommentRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_comment: Comment, old_comment: CommentRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_comment: CommentRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_comment: CommentRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[CommentRead]:
    ...
    async def find_many(
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> List[CommentRead]:
    ...
    async def from_dataframe(
        ...
        return await cls.create_many([CommentCreate.from_dict(record) for record in records_data])
