...
class RealmAsyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``Realm``.
    ...
    Attributes:
        ...
        _df_validator (RealmDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[RealmRead]:
    ...
    Métodos de escritura:
        create(realm, *operative_fields, session) → RealmRead:
    ...
    Métodos de upsert:
        upsert(realm, match_fields, session) → RealmRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(RealmCreate(...))
        dao.update(name=nuevo.name, updated_values=RealmUpdateValues(...))
        dao.delete(name=nuevo.name)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(RealmCreate(...), session=session)
            b = dao.create(RealmCreate(...), session=session)
        ...
        df.to_csv("realm.csv", index=False)
    ...
    @classmethod
    async def _on_create_before(cls, new_realm: Realm, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_create_after(cls, new_realm: Realm, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_update_before(cls, new_realm: Realm, old_realm: RealmRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_realm: Realm, old_realm: RealmRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_realm: RealmRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_realm: RealmRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando Realm:")
        ...
        query = query.where(Realm.name == name)

        # Aplicar regla RLS (si existe)
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, Realm, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(Realm, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[RealmRead]:
            result = await session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[auth] ✅ Realm encontrado exitosamente")
                return RealmRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[auth] 📭 Realm no encontrado")
                return None
    ...
    async def find_many(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando múltiples Realm:")
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(Realm, column_name):
                    column = getattr(Realm, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[auth] ⚠️ Columna '{column_name}' no existe en modelo Realm, ignorando en order_by")
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> List[RealmRead]:
            results = await session.execute(query)
            instances = results.scalars().all()
            
            _log_commit(session, f"[auth] ✅ Encontrados {len(instances)} registros Realm")

            return [
                RealmRead.from_instance(
                    instance, 
                    includes=includes, 
                    max_depth=5
                ) 
    ...
    async def count(
        ...
        """
        _log_tx(session, f"[auth] 🔢 Contando registros Realm con filtros aplicados")
        
        query = select(func.count()).select_from(Realm)
        ...
        _log_commit(session, f"[auth] ✅ Conteo Realm completado: {count_result} registros")
        return count_result
    ...
    async def exists(
        ...
        """
        _log_tx(session, f"[auth] ❓ Verificando existencia de registros Realm")
        ...
        _log_commit(session, f"[auth] ✅ Verificación Realm completada: {'existe' if exists_result else 'no existe'}")
    ...
    async def as_dataframe(
        ...
        Example:
            ...
            # Exportar a CSV
            df.to_csv('realm_data.csv', index=False)
            ```
        """
        try:
            import pandas as pd
    ...
    async def from_dataframe(
        ...
        if mode == 'create':
            return await cls.create_many([RealmCreate.from_dict(record) for record in records_data])
        else:  # upsert
            return await cls.upsert_many([RealmCreate.from_dict(record) for record in records_data], match_fields=match_fields)
    ...
    async def create(
        ...
        Args:
            realm: Datos del realm a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[auth] 🆕 Creando nuevo Realm")
        new_instance = realm.to_instance()

        async def _execute_create(session: AsyncSession):
            included = get_included_relations(new_instance, realm)
            await cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            await session.flush()
            await cls._on_create_after(new_instance, included, session)
            return RealmRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[auth] ✅ Realm creado exitosamente con name={data.name}")
        return data
    ...
    async def create_many(
        cls, 
        records: List[RealmCreate], 
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        Crea múltiples registros en la tabla realm.
        
        Args:
            records: Lista de RealmCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[auth] 🔢 Creando {len(records)} registros Realm")
        ...
        _log_commit(session, f"[auth] ✅ {result} registros Realm creados exitosamente")
    ...
    async def update(
        cls, 
        name: str,
        updated_values: RealmUpdateValues,
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[auth] 🔄 Actualizando Realm:")
        _log_tx(session, f"[auth]     name={name}")
        ...
        async def _execute_update(session: AsyncSession) -> int:
            ...
            if record is None:
                _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Realm para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = RealmRead.from_instance(record)
            ...
            if updated_values.users is not None:
                _nested = updated_values.users
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.realm_name = name
                        await UserAsyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        await UserAsyncDAO.update(
                            username=_update_dto.username,
                            realm_name=_update_dto.realm_name,
                            updated_values=_update_dto.values,
                            session=session
                        )
                if _nested.delete:
                    for _del_pk in _nested.delete:
                        await UserAsyncDAO.delete(
                            username=_del_pk['username'],
                            realm_name=_del_pk['realm_name'],
                            session=session
                        )
            if updated_values.roles is not None:
                _nested = updated_values.roles
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.realm_name = name
                        await RoleAsyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        await RoleAsyncDAO.update(
                            name=_update_dto.name,
                            realm_name=_update_dto.realm_name,
                            updated_values=_update_dto.values,
                            session=session
                        )
                if _nested.delete:
                    for _del_pk in _nested.delete:
                        await RoleAsyncDAO.delete(
                            name=_del_pk['name'],
                            realm_name=_del_pk['realm_name'],
                            session=session
                        )
            if updated_values.permissions is not None:
                _nested = updated_values.permissions
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.realm_name = name
                        await PermissionAsyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        await PermissionAsyncDAO.update(
                            name=_update_dto.name,
                            realm_name=_update_dto.realm_name,
                            updated_values=_update_dto.values,
                            session=session
                ...
                if _nested.delete:
                    for _del_pk in _nested.delete:
                        await PermissionAsyncDAO.delete(
                            name=_del_pk['name'],
                            realm_name=_del_pk['realm_name'],
                            session=session
                        )
        ...
        if result > 0:
            _log_commit(session, f"[auth] ✅ 1 registros Realm actualizados exitosamente")
        else:
            _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Realm para actualizar con los identificadores proporcionados.")
    ...
    async def update_many(
        ...
        """
        _log_tx(session, f"[auth] 🔄 Actualizando múltiples Realm con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[auth] ✅ {result.rowcount} registros Realm actualizados masivamente exitosamente")
    ...
    async def delete(
        ...
        """
        _log_tx(session, f"[auth] 🗑️ Eliminando Realm:")
        _log_tx(session, f"[auth]    name={name}")

        async def _execute_delete(session: AsyncSession) -> int:
            fetch_query = select(Realm)
            fetch_query = fetch_query.where(Realm.name == name)
            ...
            del_query = delete(Realm)
            del_query = del_query.where(Realm.name == name)
        ...
        _log_commit(session, f"[auth] ✅ {deleted} registros Realm eliminados exitosamente")
    ...
    async def delete_many(
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            _log_tx(session, f"[auth] 🗑️  Eliminando múltiples Realm con filtros: {filters}")
        ...
        _log_commit(session, f"[auth] ✅ {total_deleted} registros Realm eliminados masivamente exitosamente")
    ...
    async def _build_native_upsert_stmt(cls, dialect_name: str, values: Union[Dict[str, Any], List[Dict[str, Any]]], match_fields: List[str], update_columns: List[str]):
        ...
        if dialect_name in ('postgresql', 'sqlite'):
            ...
            stmt = dialect_insert(Realm).values(values)
            if update_columns:
            ...
            stmt = mysql_insert(Realm).values(values)
            if update_columns:
    ...
    async def upsert(
        ...
        Args:
            realm: Datos del registro a insertar o actualizar
        ...
        """
        _log_tx(session, f"[auth] 🔄 Upsert Realm con campos de coincidencia: {match_fields}, datos: {realm.to_dict()}")

        record_dict = realm.to_dict()
        ...
        for field in match_fields:
            if field not in record_dict:
                raise ValueError(f"El campo '{field}' debe estar presente en 'realm' para upsert.")
        ...
        async def execute_query(session: AsyncSession) -> RealmRead:
            # SELECT para determinar si es create o update
            query = select(Realm)
            for field in match_fields:
                query = query.where(getattr(Realm, field) == record_dict[field])
            existing = await session.execute(query).scalars().first()

            if existing:
                update_values = RealmUpdateValues(**{k: record_dict[k] for k in update_columns if k in record_dict})
                await cls.update(
                    name=existing.name,
                ...
                return RealmRead.from_instance(result)
            else:
                return await cls.create(realm, session=session)

        if session is not None:
            result = await execute_query(session)
            _log_commit(session, f"[auth] ✅ Upsert Realm completado exitosamente")
            return result
        else:
            async with async_session_manager.get_session() as session:
                result = await execute_query(session)
    ...
    async def upsert_many(
        ...
        _log_tx(session, f"[auth] 🔄 Upsert masivo Realm con {len(records)} registros, campos de coincidencia: {match_fields}")
                ...
                if stmt is not None:
                    ...
                    for record in batch:
                        if len(match_fields) == 1:
                            ...
                            conditions.append(getattr(Realm, field) == record[field])
                        else:
                            record_conds = [getattr(Realm, f) == record[f] for f in match_fields]
                            conditions.append(and_(*record_conds))
                    
                    query = select(Realm).where(or_(*conditions))
                    ...
                    if to_insert:
                        await session.execute(Realm.__table__.insert(), to_insert)
                    
                    # Bulk update de registros existentes
                    for record in to_update:
                        update_stmt = update(Realm)
                        for field in match_fields:
                            update_stmt = update_stmt.where(getattr(Realm, field) == record[field])
        ...
        _log_commit(session, f"[auth] ✅ Upsert masivo Realm completado: {result} registros procesados")
    ...
    async def sum(
        ...
        """
        _log_tx(session, f"[auth] 🧮 Sumando campos {agg_fields} de registros Realm con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(Realm, field):
                column = getattr(Realm, field)
                ...
                error_msg = f"Campo '{field}' no existe en modelo Realm"
        ...
        _log_commit(session, f"[auth] ✅ Suma Realm completada: {sum_result}")
    ...
    async def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 📊 Calculando media de campos {agg_fields} de registros Realm con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Media Realm completada: {mean_result}")
    ...
    async def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔺 Calculando máximo de campos {agg_fields} de registros Realm con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Máximo Realm completado: {max_result}")
    ...
    async def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔻 Calculando mínimo de campos {agg_fields} de registros Realm con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Mínimo Realm completado: {min_result}")
    ...
    async def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[auth] 🎯 Realizando agregaciones en Realm: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, Realm)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(Realm, expression):
                            column      = getattr(Realm, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo Realm")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(Realm, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo Realm")
                    continue

                column      = getattr(Realm, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[auth] ✅ Agregaciones Realm completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
