...
class UserRoleAsyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``UserRole``.
    ...
    Attributes:
        ...
        _df_validator (UserRoleDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[UserRoleRead]:
    ...
    Métodos de escritura:
        create(user_role, *operative_fields, session) → UserRoleRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(UserRoleCreate(...))
        dao.update(id=nuevo.id, updated_values=UserRoleUpdateValues(...))
        dao.delete(id=nuevo.id)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(UserRoleCreate(...), session=session)
            b = dao.create(UserRoleCreate(...), session=session)
        ...
        df.to_csv("user_role.csv", index=False)
    ...
    @classmethod
    async def _on_create_before(cls, new_user_role: UserRole, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_create_after(cls, new_user_role: UserRole, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_update_before(cls, new_user_role: UserRole, old_user_role: UserRoleRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_user_role: UserRole, old_user_role: UserRoleRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_user_role: UserRoleRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_user_role: UserRoleRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando UserRole:")
        ...
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, UserRole, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(UserRole, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[UserRoleRead]:
            result = await session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[auth] ✅ UserRole encontrado exitosamente")
                return UserRoleRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[auth] 📭 UserRole no encontrado")
                return None
    ...
    async def find_many(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando múltiples UserRole:")
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(UserRole, column_name):
                    column = getattr(UserRole, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[auth] ⚠️ Columna '{column_name}' no existe en modelo UserRole, ignorando en order_by")
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> List[UserRoleRead]:
            results = await session.execute(query)
            instances = results.scalars().all()
            
            _log_commit(session, f"[auth] ✅ Encontrados {len(instances)} registros UserRole")

            return [
                UserRoleRead.from_instance(
                    instance, 
                    includes=includes, 
                    max_depth=5
                ) 
    ...
    async def count(
        ...
        """
        _log_tx(session, f"[auth] 🔢 Contando registros UserRole con filtros aplicados")
        
        query = select(func.count()).select_from(UserRole)
        ...
        _log_commit(session, f"[auth] ✅ Conteo UserRole completado: {count_result} registros")
        return count_result
    ...
    async def exists(
        ...
        """
        _log_tx(session, f"[auth] ❓ Verificando existencia de registros UserRole")
        ...
        _log_commit(session, f"[auth] ✅ Verificación UserRole completada: {'existe' if exists_result else 'no existe'}")
    ...
    async def as_dataframe(
        ...
        Example:
            ...
            df = db_api.userrole.as_dataframe()
            ...
            # Exportar a CSV
            df.to_csv('userrole_data.csv', index=False)
            ```
        """
        try:
            import pandas as pd
    ...
    async def from_dataframe(
        ...
        Example:
            ...
            crud = UserRoleAsyncDAO
            
            # Crear DataFrame
            df = pd.DataFrame({
                'id': [1, 2, 3],
                'user_name': ['valor1', 'valor2', 'valor3'],
        ...
        return await cls.create_many([UserRoleCreate.from_dict(record) for record in records_data])
    ...
    async def create(
        ...
        Args:
            user_role: Datos del user_role a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[auth] 🆕 Creando nuevo UserRole")
        new_instance = user_role.to_instance()

        async def _execute_create(session: AsyncSession):
            included = get_included_relations(new_instance, user_role)
            await cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            await session.flush()
            await cls._on_create_after(new_instance, included, session)
            return UserRoleRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[auth] ✅ UserRole creado exitosamente con id={data.id}")
        return data
    ...
    async def create_many(
        cls, 
        records: List[UserRoleCreate], 
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        Crea múltiples registros en la tabla user_role.
        
        Args:
            records: Lista de UserRoleCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[auth] 🔢 Creando {len(records)} registros UserRole")
        ...
        _log_commit(session, f"[auth] ✅ {result} registros UserRole creados exitosamente")
    ...
    async def update(
        cls, 
        id: int,
        updated_values: UserRoleUpdateValues,
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[auth] 🔄 Actualizando UserRole:")
        _log_tx(session, f"[auth]     id={id}")
        ...
        async def _execute_update(session: AsyncSession) -> int:
            ...
            if record is None:
                _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro UserRole para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = UserRoleRead.from_instance(record)
        ...
        if result > 0:
            _log_commit(session, f"[auth] ✅ 1 registros UserRole actualizados exitosamente")
        else:
            _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro UserRole para actualizar con los identificadores proporcionados.")
    ...
    async def update_many(
        ...
        """
        _log_tx(session, f"[auth] 🔄 Actualizando múltiples UserRole con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[auth] ✅ {result.rowcount} registros UserRole actualizados masivamente exitosamente")
    ...
    async def delete(
        ...
        """
        _log_tx(session, f"[auth] 🗑️ Eliminando UserRole:")
        _log_tx(session, f"[auth]    id={id}")

        async def _execute_delete(session: AsyncSession) -> int:
            fetch_query = select(UserRole)
            fetch_query = fetch_query.where(UserRole.id == id)
            ...
            del_query = delete(UserRole)
            del_query = del_query.where(UserRole.id == id)
        ...
        _log_commit(session, f"[auth] ✅ {deleted} registros UserRole eliminados exitosamente")
    ...
    async def delete_many(
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            _log_tx(session, f"[auth] 🗑️  Eliminando múltiples UserRole con filtros: {filters}")
        ...
        _log_commit(session, f"[auth] ✅ {total_deleted} registros UserRole eliminados masivamente exitosamente")
    ...
    async def sum(
        ...
        """
        _log_tx(session, f"[auth] 🧮 Sumando campos {agg_fields} de registros UserRole con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(UserRole, field):
                column = getattr(UserRole, field)
                ...
                error_msg = f"Campo '{field}' no existe en modelo UserRole"
        ...
        _log_commit(session, f"[auth] ✅ Suma UserRole completada: {sum_result}")
    ...
    async def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 📊 Calculando media de campos {agg_fields} de registros UserRole con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Media UserRole completada: {mean_result}")
    ...
    async def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔺 Calculando máximo de campos {agg_fields} de registros UserRole con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Máximo UserRole completado: {max_result}")
    ...
    async def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔻 Calculando mínimo de campos {agg_fields} de registros UserRole con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Mínimo UserRole completado: {min_result}")
    ...
    async def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[auth] 🎯 Realizando agregaciones en UserRole: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, UserRole)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(UserRole, expression):
                            column      = getattr(UserRole, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo UserRole")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(UserRole, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo UserRole")
                    continue

                column      = getattr(UserRole, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[auth] ✅ Agregaciones UserRole completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
