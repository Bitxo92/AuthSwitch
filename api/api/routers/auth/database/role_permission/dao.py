...
class RolePermissionAsyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``RolePermission``.
    ...
    Attributes:
        ...
        _df_validator (RolePermissionDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[RolePermissionRead]:
    ...
    Métodos de escritura:
        create(role_permission, *operative_fields, session) → RolePermissionRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(RolePermissionCreate(...))
        dao.update(id=nuevo.id, updated_values=RolePermissionUpdateValues(...))
        dao.delete(id=nuevo.id)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(RolePermissionCreate(...), session=session)
            b = dao.create(RolePermissionCreate(...), session=session)
        ...
        df.to_csv("role_permission.csv", index=False)
    ...
    @classmethod
    async def _on_create_before(cls, new_role_permission: RolePermission, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_create_after(cls, new_role_permission: RolePermission, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_update_before(cls, new_role_permission: RolePermission, old_role_permission: RolePermissionRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_role_permission: RolePermission, old_role_permission: RolePermissionRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_role_permission: RolePermissionRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_role_permission: RolePermissionRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando RolePermission:")
        _log_tx(session, f"[auth]     id={id}")
        _log_tx(session, f"[auth]     includes={includes}")

        # Construir query base
        query = select(RolePermission)
        
        query = query.where(RolePermission.id == id)

        # Aplicar regla RLS (si existe)
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, RolePermission, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(RolePermission, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[RolePermissionRead]:
            result = await session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[auth] ✅ RolePermission encontrado exitosamente")
                return RolePermissionRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[auth] 📭 RolePermission no encontrado")
                return None
    ...
    async def find_many(
        ...
        in_permission_name: Optional[List[str]] = None,
        ...
        - role_name: Filtrar por role_name
        - in_role_name: Filtrar por múltiples valores de role_name (OR lógico)
        - permission_name: Filtrar por permission_name
        - in_permission_name: Filtrar por múltiples valores de permission_name (OR lógico)
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando múltiples RolePermission:")
        ...
        _filter = RolePermissionFilter(
            role_name=role_name,
            in_role_name=in_role_name,
            permission_name=permission_name,
            in_permission_name=in_permission_name,
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(RolePermission, column_name):
                    column = getattr(RolePermission, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[auth] ⚠️ Columna '{column_name}' no existe en modelo RolePermission, ignorando en order_by")
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> List[RolePermissionRead]:
            results = await session.execute(query)
            instances = results.scalars().all()
            
            _log_commit(session, f"[auth] ✅ Encontrados {len(instances)} registros RolePermission")

            return [
                RolePermissionRead.from_instance(
                    instance, 
                    includes=includes, 
                    max_depth=5
                ) 
    ...
    async def count(
        ...
        """
        _log_tx(session, f"[auth] 🔢 Contando registros RolePermission con filtros aplicados")
        
        query = select(func.count()).select_from(RolePermission)
        ...
        _log_commit(session, f"[auth] ✅ Conteo RolePermission completado: {count_result} registros")
        return count_result
    ...
    async def exists(
        ...
        """
        _log_tx(session, f"[auth] ❓ Verificando existencia de registros RolePermission")
        ...
        _log_commit(session, f"[auth] ✅ Verificación RolePermission completada: {'existe' if exists_result else 'no existe'}")
    ...
    async def as_dataframe(
        ...
        Example:
            ...
            df = db_api.rolepermission.as_dataframe()
            ...
            # Exportar a CSV
            df.to_csv('rolepermission_data.csv', index=False)
            ```
        """
        try:
            import pandas as pd
    ...
    async def from_dataframe(
        ...
        Example:
            ...
            crud = RolePermissionAsyncDAO
            
            # Crear DataFrame
            df = pd.DataFrame({
                'id': [1, 2, 3],
                'role_name': ['valor1', 'valor2', 'valor3'],
                'permission_name': ['valor1', 'valor2', 'valor3'],
        ...
        return await cls.create_many([RolePermissionCreate.from_dict(record) for record in records_data])
    ...
    async def create(
        ...
        Args:
            role_permission: Datos del role_permission a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[auth] 🆕 Creando nuevo RolePermission")
        new_instance = role_permission.to_instance()

        async def _execute_create(session: AsyncSession):
            included = get_included_relations(new_instance, role_permission)
            await cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            await session.flush()
            await cls._on_create_after(new_instance, included, session)
            return RolePermissionRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[auth] ✅ RolePermission creado exitosamente con id={data.id}")
        return data
    ...
    async def create_many(
        cls, 
        records: List[RolePermissionCreate], 
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        Crea múltiples registros en la tabla role_permission.
        
        Args:
            records: Lista de RolePermissionCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[auth] 🔢 Creando {len(records)} registros RolePermission")
        ...
        _log_commit(session, f"[auth] ✅ {result} registros RolePermission creados exitosamente")
    ...
    async def update(
        cls, 
        id: int,
        updated_values: RolePermissionUpdateValues,
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[auth] 🔄 Actualizando RolePermission:")
        _log_tx(session, f"[auth]     id={id}")
        ...
        async def _execute_update(session: AsyncSession) -> int:
            ...
            if record is None:
                _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro RolePermission para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = RolePermissionRead.from_instance(record)
        ...
        if result > 0:
            _log_commit(session, f"[auth] ✅ 1 registros RolePermission actualizados exitosamente")
        else:
            _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro RolePermission para actualizar con los identificadores proporcionados.")
    ...
    async def update_many(
        ...
        """
        _log_tx(session, f"[auth] 🔄 Actualizando múltiples RolePermission con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[auth] ✅ {result.rowcount} registros RolePermission actualizados masivamente exitosamente")
    ...
    async def delete(
        ...
        """
        _log_tx(session, f"[auth] 🗑️ Eliminando RolePermission:")
        _log_tx(session, f"[auth]    id={id}")

        async def _execute_delete(session: AsyncSession) -> int:
            fetch_query = select(RolePermission)
            fetch_query = fetch_query.where(RolePermission.id == id)
            ...
            del_query = delete(RolePermission)
            del_query = del_query.where(RolePermission.id == id)
        ...
        _log_commit(session, f"[auth] ✅ {deleted} registros RolePermission eliminados exitosamente")
    ...
    async def delete_many(
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            _log_tx(session, f"[auth] 🗑️  Eliminando múltiples RolePermission con filtros: {filters}")
        ...
        _log_commit(session, f"[auth] ✅ {total_deleted} registros RolePermission eliminados masivamente exitosamente")
    ...
    async def sum(
        ...
        """
        _log_tx(session, f"[auth] 🧮 Sumando campos {agg_fields} de registros RolePermission con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(RolePermission, field):
                column = getattr(RolePermission, field)
                ...
                error_msg = f"Campo '{field}' no existe en modelo RolePermission"
        ...
        _log_commit(session, f"[auth] ✅ Suma RolePermission completada: {sum_result}")
    ...
    async def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 📊 Calculando media de campos {agg_fields} de registros RolePermission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Media RolePermission completada: {mean_result}")
    ...
    async def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔺 Calculando máximo de campos {agg_fields} de registros RolePermission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Máximo RolePermission completado: {max_result}")
    ...
    async def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔻 Calculando mínimo de campos {agg_fields} de registros RolePermission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Mínimo RolePermission completado: {min_result}")
    ...
    async def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[auth] 🎯 Realizando agregaciones en RolePermission: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, RolePermission)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(RolePermission, expression):
                            column      = getattr(RolePermission, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo RolePermission")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(RolePermission, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo RolePermission")
                    continue

                column      = getattr(RolePermission, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[auth] ✅ Agregaciones RolePermission completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
