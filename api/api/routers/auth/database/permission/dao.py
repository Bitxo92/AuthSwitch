...
class PermissionAsyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``Permission``.
    ...
    Attributes:
        ...
        _df_validator (PermissionDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[PermissionRead]:
    ...
    Métodos de escritura:
        create(permission, *operative_fields, session) → PermissionRead:
    ...
    Métodos de upsert:
        upsert(permission, match_fields, session) → PermissionRead:
    ...
    Ejemplos de uso:
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(PermissionCreate(...))
        dao.update(name=nuevo.name, realm_name=nuevo.realm_name, updated_values=PermissionUpdateValues(...))
        dao.delete(name=nuevo.namerealm_name=nuevo.realm_name)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(PermissionCreate(...), session=session)
            b = dao.create(PermissionCreate(...), session=session)
        ...
        df.to_csv("permission.csv", index=False)
    ...
    @classmethod
    async def _on_create_before(cls, new_permission: Permission, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_create_after(cls, new_permission: Permission, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_update_before(cls, new_permission: Permission, old_permission: PermissionRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_permission: Permission, old_permission: PermissionRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_permission: PermissionRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_permission: PermissionRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando Permission:")
        ...
        query = query.where(Permission.realm_name == realm_name)

        # Aplicar regla RLS (si existe)
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, Permission, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(Permission, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[PermissionRead]:
            result = await session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[auth] ✅ Permission encontrado exitosamente")
                return PermissionRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[auth] 📭 Permission no encontrado")
                return None
    ...
    async def find_many(
        ...
        - in_type: Filtrar por múltiples valores de type (OR lógico)
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando múltiples Permission:")
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(Permission, column_name):
                    column = getattr(Permission, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[auth] ⚠️ Columna '{column_name}' no existe en modelo Permission, ignorando en order_by")
        ...
        # Ejecutar query
        async def execute_query(session: AsyncSession) -> List[PermissionRead]:
            results = await session.execute(query)
            instances = results.scalars().all()
            
            _log_commit(session, f"[auth] ✅ Encontrados {len(instances)} registros Permission")

            return [
                PermissionRead.from_instance(
                    instance, 
                    includes=includes, 
                    max_depth=5
                ) 
    ...
    async def count(
        ...
        """
        _log_tx(session, f"[auth] 🔢 Contando registros Permission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Conteo Permission completado: {count_result} registros")
        return count_result
    ...
    async def exists(
        ...
        """
        _log_tx(session, f"[auth] ❓ Verificando existencia de registros Permission")
        ...
        _log_commit(session, f"[auth] ✅ Verificación Permission completada: {'existe' if exists_result else 'no existe'}")
    ...
    async def as_dataframe(
        ...
        Example:
            ...
            df = db_api.permission.as_dataframe()
            ...
            # Exportar a CSV
            df.to_csv('permission_data.csv', index=False)
            ```
        """
        try:
            import pandas as pd
    ...
    async def from_dataframe(
        ...
        if mode == 'create':
            return await cls.create_many([PermissionCreate.from_dict(record) for record in records_data])
        else:  # upsert
            return await cls.upsert_many([PermissionCreate.from_dict(record) for record in records_data], match_fields=match_fields)
    ...
    async def create(
        ...
        Args:
            permission: Datos del permission a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[auth] 🆕 Creando nuevo Permission")
        new_instance = permission.to_instance()

        async def _execute_create(session: AsyncSession):
            included = get_included_relations(new_instance, permission)
            await cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            await session.flush()
            await cls._on_create_after(new_instance, included, session)
            return PermissionRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[auth] ✅ Permission creado exitosamente con name={data.name}, realm_name={data.realm_name}")
        return data
    ...
    async def create_many(
        cls, 
        records: List[PermissionCreate], 
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        Crea múltiples registros en la tabla permission.
        
        Args:
            records: Lista de PermissionCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[auth] 🔢 Creando {len(records)} registros Permission")
        ...
        _log_commit(session, f"[auth] ✅ {result} registros Permission creados exitosamente")
    ...
    async def update(
        ...
        name: str,
        realm_name: str,
        updated_values: PermissionUpdateValues,
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[auth] 🔄 Actualizando Permission:")
        _log_tx(session, f"[auth]     name={name}")
        _log_tx(session, f"[auth]     realm_name={realm_name}")
        ...
        async def _execute_update(session: AsyncSession) -> int:
            ...
            if record is None:
                _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Permission para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = PermissionRead.from_instance(record)
            ...
            if updated_values.role_permissions is not None:
                ...
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.permission_name = name
        ...
        if result > 0:
            _log_commit(session, f"[auth] ✅ 1 registros Permission actualizados exitosamente")
        else:
            _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Permission para actualizar con los identificadores proporcionados.")
    ...
    async def update_many(
        ...
        """
        _log_tx(session, f"[auth] 🔄 Actualizando múltiples Permission con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[auth] ✅ {result.rowcount} registros Permission actualizados masivamente exitosamente")
    ...
    async def delete(
        ...
        """
        _log_tx(session, f"[auth] 🗑️ Eliminando Permission:")
        _log_tx(session, f"[auth]    name={name}")
        _log_tx(session, f"[auth]    realm_name={realm_name}")

        async def _execute_delete(session: AsyncSession) -> int:
            fetch_query = select(Permission)
            fetch_query = fetch_query.where(Permission.name == name)
            fetch_query = fetch_query.where(Permission.realm_name == realm_name)
            ...
            del_query = delete(Permission)
            del_query = del_query.where(Permission.name == name)
            del_query = del_query.where(Permission.realm_name == realm_name)
        ...
        _log_commit(session, f"[auth] ✅ {deleted} registros Permission eliminados exitosamente")
    ...
    async def delete_many(
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            _log_tx(session, f"[auth] 🗑️  Eliminando múltiples Permission con filtros: {filters}")
        ...
        _log_commit(session, f"[auth] ✅ {total_deleted} registros Permission eliminados masivamente exitosamente")
    ...
    async def _build_native_upsert_stmt(cls, dialect_name: str, values: Union[Dict[str, Any], List[Dict[str, Any]]], match_fields: List[str], update_columns: List[str]):
        ...
        if dialect_name in ('postgresql', 'sqlite'):
            ...
            stmt = dialect_insert(Permission).values(values)
            if update_columns:
            ...
            stmt = mysql_insert(Permission).values(values)
            if update_columns:
    ...
    async def upsert(
        ...
        Args:
            permission: Datos del registro a insertar o actualizar
        ...
        """
        _log_tx(session, f"[auth] 🔄 Upsert Permission con campos de coincidencia: {match_fields}, datos: {permission.to_dict()}")

        record_dict = permission.to_dict()
        ...
        for field in match_fields:
            if field not in record_dict:
                raise ValueError(f"El campo '{field}' debe estar presente en 'permission' para upsert.")
        ...
        async def execute_query(session: AsyncSession) -> PermissionRead:
            # SELECT para determinar si es create o update
            query = select(Permission)
            for field in match_fields:
                query = query.where(getattr(Permission, field) == record_dict[field])
            existing = await session.execute(query).scalars().first()

            if existing:
                update_values = PermissionUpdateValues(**{k: record_dict[k] for k in update_columns if k in record_dict})
                await cls.update(
                    name=existing.name,
                ...
                return PermissionRead.from_instance(result)
            else:
                return await cls.create(permission, session=session)

        if session is not None:
            result = await execute_query(session)
            _log_commit(session, f"[auth] ✅ Upsert Permission completado exitosamente")
            return result
        else:
            async with async_session_manager.get_session() as session:
                result = await execute_query(session)
    ...
    async def upsert_many(
        ...
        _log_tx(session, f"[auth] 🔄 Upsert masivo Permission con {len(records)} registros, campos de coincidencia: {match_fields}")
                ...
                if stmt is not None:
                    ...
                    for record in batch:
                        if len(match_fields) == 1:
                            ...
                            conditions.append(getattr(Permission, field) == record[field])
                        else:
                            record_conds = [getattr(Permission, f) == record[f] for f in match_fields]
                            conditions.append(and_(*record_conds))
                    
                    query = select(Permission).where(or_(*conditions))
                    ...
                    if to_insert:
                        await session.execute(Permission.__table__.insert(), to_insert)
                    
                    # Bulk update de registros existentes
                    for record in to_update:
                        update_stmt = update(Permission)
                        for field in match_fields:
                            update_stmt = update_stmt.where(getattr(Permission, field) == record[field])
        ...
        _log_commit(session, f"[auth] ✅ Upsert masivo Permission completado: {result} registros procesados")
    ...
    async def sum(
        ...
        """
        _log_tx(session, f"[auth] 🧮 Sumando campos {agg_fields} de registros Permission con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(Permission, field):
                column = getattr(Permission, field)
                ...
                error_msg = f"Campo '{field}' no existe en modelo Permission"
        ...
        _log_commit(session, f"[auth] ✅ Suma Permission completada: {sum_result}")
    ...
    async def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 📊 Calculando media de campos {agg_fields} de registros Permission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Media Permission completada: {mean_result}")
    ...
    async def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔺 Calculando máximo de campos {agg_fields} de registros Permission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Máximo Permission completado: {max_result}")
    ...
    async def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔻 Calculando mínimo de campos {agg_fields} de registros Permission con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Mínimo Permission completado: {min_result}")
    ...
    async def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[auth] 🎯 Realizando agregaciones en Permission: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, Permission)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(Permission, expression):
                            column      = getattr(Permission, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo Permission")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(Permission, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo Permission")
                    continue

                column      = getattr(Permission, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[auth] ✅ Agregaciones Permission completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
