...
class RLSPredicateBuilder:
    ...
    def build_predicate(
        ...
        try:
            ...
            logger.error(f"[auth] Column {target_column} not found in {target_model.__name__}")
            return None
    ...
    def _build_base_predicate(
        ...
        try:
            if operator == RLSOperator.IN:
                ...
                logger.error(f"[auth] Unsupported RLS operator: {operator}")
                return None
        except Exception as e:
            logger.error(f"[auth] Error building base predicate: {e}")
            return None
    ...
    def _build_relationship_predicate(
        ...
        try:
            ...
            logger.error(f"[auth] Error building relationship predicate: {e}")
            return None
    ...
    def apply_rls(
        ...
        for rule in rules:
            ...
            if predicate is not None:
                logger.info(f"[auth] Applying RLS to {model_cls.__name__}")
                logger.info(f"[auth]     Rule: {rule}")
