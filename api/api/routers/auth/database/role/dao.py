...
class RoleAsyncDAO:
    """
    DAO síncrono de acceso a datos para el modelo ``Role``.
    ...
    Attributes:
        ...
        _df_validator (RoleDataFrameValidator):
            Validador y normalizador de DataFrames para carga masiva.

    Métodos de lectura:
        find(*pk_fields, includes, rls, session) → Optional[RoleRead]:
    ...
    Métodos de escritura:
        create(role, *operative_fields, session) → RoleRead:
    ...
    Métodos de upsert:
        upsert(role, match_fields, session) → RoleRead:
    ...
    Ejemplos de uso:
        ...
        # ── Lectura ───────────────────────────────────────────────────────────
        registro  = dao.find(name=1realm_name=1)
        ...
        # ── Escritura ─────────────────────────────────────────────────────────
        nuevo = dao.create(RoleCreate(...))
        dao.update(name=nuevo.name, realm_name=nuevo.realm_name, updated_values=RoleUpdateValues(...))
        dao.delete(name=nuevo.namerealm_name=nuevo.realm_name)

        # Transacción multi-paso (rollback automático ante cualquier error)
        with session_manager.transaction() as session:
            a = dao.create(RoleCreate(...), session=session)
            b = dao.create(RoleCreate(...), session=session)
    ...
    @classmethod
    async def _on_create_before(cls, new_role: Role, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_create_after(cls, new_role: Role, included: List[str], session: AsyncSession):
        pass

    @classmethod
    async def _on_update_before(cls, new_role: Role, old_role: RoleRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_update_after(cls, new_role: Role, old_role: RoleRead, update_data: dict, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_before(cls, old_role: RoleRead, session: AsyncSession):
        pass

    @classmethod
    async def _on_delete_after(cls, old_role: RoleRead, session: AsyncSession):
        pass
    ...
    async def find(
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando Role:")
        _log_tx(session, f"[auth]     name={name}")
        ...
        query = query.where(Role.realm_name == realm_name)

        # Aplicar regla RLS (si existe)
        if rls is not None:
            query = RLSQueryApplicator.apply_rls(query, Role, rls)

        # Aplicar opciones de carga optimizada
        if includes:
            loading_options = get_loading_options(Role, includes)
            if loading_options:
                query = query.options(*loading_options)


        # Ejecutar query
        async def execute_query(session: AsyncSession) -> Optional[RoleRead]:
            result = await session.execute(query)
            instance = result.scalars().first()
            
            if instance:
                _log_commit(session, f"[auth] ✅ Role encontrado exitosamente")
                return RoleRead.from_instance(
                    instance,
                ...
                _log_commit(session, f"[auth] 📭 Role no encontrado")
                return None
    ...
    async def find_many(
        ...
        - in_description: Filtrar por múltiples valores de description (OR lógico)
        - parent_role_name: Filtrar por parent_role_name
        - in_parent_role_name: Filtrar por múltiples valores de parent_role_name (OR lógico)
        ...
        """
        _log_tx(session, f"[auth] 🔍 Buscando múltiples Role:")
        ...
        _filter = RoleFilter(
            ...
            in_parent_role_name=in_parent_role_name,
        )
        ...
        if _hierarchy_mode != TreeMode.FLAT:
            if tree_root is not None:
                ...
                async def _get_root_ids(_sess):
                    _root_result = await _sess.execute(query)
                    return [row.name for row in _root_result.scalars().all()]

                if session is not None:
                    _hierarchy_root_ids = await _get_root_ids(session)
                else:
                    async with async_session_manager.get_session() as _tmp_session:
                        _hierarchy_root_ids = await _get_root_ids(_tmp_session)
        ...
        if order_by:
            for column_name in order_by:
                if hasattr(Role, column_name):
                    column = getattr(Role, column_name)
                    if order.upper() == "DESC":
                        query = query.order_by(column.desc())
                    elif order.upper() == "ASC":
                        query = query.order_by(column.asc())
                else:
                    logger.warning(f"[auth] ⚠️ Columna '{column_name}' no existe en modelo Role, ignorando en order_by")
...
# ── Hierarchy resolver instance ───────────────────────────────────────
        _hierarchy_resolver = HierarchyResolver(
            config=HierarchyConfig(Role, 'name', 'parent_role_name'),
            dto_factory=lambda instance, includes, max_depth: RoleRead.from_instance(
                instance,
                includes=includes,
                max_depth=max_depth
            ),
        ...
        async def execute_query(session: AsyncSession) -> List[RoleRead]:
            if _hierarchy_mode == TreeMode.FLAT:
                # Standard flat query
                results = await session.execute(query)
                instances = results.scalars().all()

                _log_commit(session, f"[auth] ✅ Encontrados {len(instances)} registros Role")

                return [
                    RoleRead.from_instance(
                        instance,
                        includes=includes,
                        max_depth=5
                    )
            ...
            _log_commit(session, f"[auth] ✅ Encontrados {len(result_dtos)} registros Role (tree_mode={_hierarchy_mode.value})")
            return result_dtos
    ...
    async def count(
        ...
        """
        _log_tx(session, f"[auth] 🔢 Contando registros Role con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Conteo Role completado: {count_result} registros")
        return count_result
    ...
    async def exists(
        ...
        """
        _log_tx(session, f"[auth] ❓ Verificando existencia de registros Role")
        ...
        _log_commit(session, f"[auth] ✅ Verificación Role completada: {'existe' if exists_result else 'no existe'}")
    ...
    async def as_dataframe(
        ...
        Example:
            ...
            # Exportar a CSV
            df.to_csv('role_data.csv', index=False)
            ```
        """
        try:
            import pandas as pd
    ...
    async def from_dataframe(
        ...
        Example:
            ...
            df = pd.DataFrame({
                ...
                'description': ['valor1', 'valor2', 'valor3'],
                'attributes': [None, None, None],
                'parent_role_name': ['valor1', 'valor2', 'valor3']
            })
        ...
        if mode == 'create':
            return await cls.create_many([RoleCreate.from_dict(record) for record in records_data])
        else:  # upsert
            return await cls.upsert_many([RoleCreate.from_dict(record) for record in records_data], match_fields=match_fields)
    ...
    async def create(
        ...
        Args:
            role: Datos del role a crear
            session: Sesión existente (opcional)
            
        Returns:
            Instancia del modelo creado
        """
        _log_tx(session, f"[auth] 🆕 Creando nuevo Role")
        new_instance = role.to_instance()

        async def _execute_create(session: AsyncSession):
            included = get_included_relations(new_instance, role)
            await cls._on_create_before(new_instance, included, session)
            session.add(new_instance)
            await session.flush()
            await cls._on_create_after(new_instance, included, session)
            return RoleRead.from_created_instance(new_instance, included)
        ...
        _log_commit(session, f"[auth] ✅ Role creado exitosamente con name={data.name}, realm_name={data.realm_name}")
        return data
    ...
    async def create_many(
        ...
        """
        Crea múltiples registros en la tabla role.
        
        Args:
            records: Lista de RoleCreate con los datos de los registros
            session: Sesión existente (opcional)
        ...
        """
        _log_tx(session, f"[auth] 🔢 Creando {len(records)} registros Role")
        ...
        _log_commit(session, f"[auth] ✅ {result} registros Role creados exitosamente")
    ...
    async def update(
        ...
        name: str,
        realm_name: str,
        updated_values: RoleUpdateValues,
        session: Optional[AsyncSession] = None
    ) -> int:
        """
        ...
        _log_tx(session, f"[auth] 🔄 Actualizando Role:")
        _log_tx(session, f"[auth]     name={name}")
        _log_tx(session, f"[auth]     realm_name={realm_name}")
        ...
        async def _execute_update(session: AsyncSession) -> int:
            ...
            if record is None:
                _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Role para actualizar con los identificadores proporcionados.")
                return 0

            old_dto = RoleRead.from_instance(record)
            ...
            if updated_values.role_permissions is not None:
                _nested = updated_values.role_permissions
                if _nested.create:
                    for _create_dto in _nested.create:
                        _create_dto.role_name = name
                        _create_dto.realm_name = realm_name
                        await RolePermissionAsyncDAO.create(_create_dto, session=session)
                if _nested.update:
                    for _update_dto in _nested.update:
                        await RolePermissionAsyncDAO.update(
                            id=_update_dto.id,
                ...
                if _nested.delete:
                    for _del_pk in _nested.delete:
                        await RolePermissionAsyncDAO.delete(
                            id=_del_pk,
                            session=session
                        )
        ...
        if result > 0:
            _log_commit(session, f"[auth] ✅ 1 registros Role actualizados exitosamente")
        else:
            _log_tx(session, f"[auth] ⚠️ No se encontró ningún registro Role para actualizar con los identificadores proporcionados.")
    ...
    async def update_many(
        ...
        """
        _log_tx(session, f"[auth] 🔄 Actualizando múltiples Role con filtros: {payload.filter.to_dict()}, valores: {payload.values.to_dict()}")
        ...
        _log_commit(session, f"[auth] ✅ {result.rowcount} registros Role actualizados masivamente exitosamente")
    ...
    async def delete(
        ...
        Args:
            name: Filtrar por name para eliminar
        ...
        """
        _log_tx(session, f"[auth] 🗑️ Eliminando Role:")
        _log_tx(session, f"[auth]    name={name}")
        _log_tx(session, f"[auth]    realm_name={realm_name}")

        async def _execute_delete(session: AsyncSession) -> int:
            fetch_query = select(Role)
            fetch_query = fetch_query.where(Role.name == name)
            fetch_query = fetch_query.where(Role.realm_name == realm_name)
            ...
            del_query = delete(Role)
            del_query = del_query.where(Role.name == name)
            del_query = del_query.where(Role.realm_name == realm_name)
        ...
        _log_commit(session, f"[auth] ✅ {deleted} registros Role eliminados exitosamente")
    ...
    async def delete_many(
        ...
        async def execute_query(session: AsyncSession) -> int:
            ...
            _log_tx(session, f"[auth] 🗑️  Eliminando múltiples Role con filtros: {filters}")
        ...
        _log_commit(session, f"[auth] ✅ {total_deleted} registros Role eliminados masivamente exitosamente")
    ...
    async def _build_native_upsert_stmt(cls, dialect_name: str, values: Union[Dict[str, Any], List[Dict[str, Any]]], match_fields: List[str], update_columns: List[str]):
        ...
        if dialect_name in ('postgresql', 'sqlite'):
            ...
            stmt = dialect_insert(Role).values(values)
            if update_columns:
            ...
            stmt = mysql_insert(Role).values(values)
            if update_columns:
    ...
    async def upsert(
        ...
        Args:
            role: Datos del registro a insertar o actualizar
        ...
        """
        _log_tx(session, f"[auth] 🔄 Upsert Role con campos de coincidencia: {match_fields}, datos: {role.to_dict()}")

        record_dict = role.to_dict()
        ...
        for field in match_fields:
            if field not in record_dict:
                raise ValueError(f"El campo '{field}' debe estar presente en 'role' para upsert.")
        ...
        async def execute_query(session: AsyncSession) -> RoleRead:
            # SELECT para determinar si es create o update
            query = select(Role)
            for field in match_fields:
                query = query.where(getattr(Role, field) == record_dict[field])
            existing = await session.execute(query).scalars().first()

            if existing:
                update_values = RoleUpdateValues(**{k: record_dict[k] for k in update_columns if k in record_dict})
                await cls.update(
                    name=existing.name,
                ...
                return RoleRead.from_instance(result)
            else:
                return await cls.create(role, session=session)

        if session is not None:
            result = await execute_query(session)
            _log_commit(session, f"[auth] ✅ Upsert Role completado exitosamente")
            return result
        else:
            async with async_session_manager.get_session() as session:
                result = await execute_query(session)
    ...
    async def upsert_many(
        ...
        _log_tx(session, f"[auth] 🔄 Upsert masivo Role con {len(records)} registros, campos de coincidencia: {match_fields}")
                ...
                if stmt is not None:
                    ...
                    for record in batch:
                        if len(match_fields) == 1:
                            ...
                            conditions.append(getattr(Role, field) == record[field])
                        else:
                            record_conds = [getattr(Role, f) == record[f] for f in match_fields]
                            conditions.append(and_(*record_conds))
                    
                    query = select(Role).where(or_(*conditions))
                    ...
                    if to_insert:
                        await session.execute(Role.__table__.insert(), to_insert)
                    
                    # Bulk update de registros existentes
                    for record in to_update:
                        update_stmt = update(Role)
                        for field in match_fields:
                            update_stmt = update_stmt.where(getattr(Role, field) == record[field])
        ...
        _log_commit(session, f"[auth] ✅ Upsert masivo Role completado: {result} registros procesados")
    ...
    async def sum(
        ...
        """
        _log_tx(session, f"[auth] 🧮 Sumando campos {agg_fields} de registros Role con filtros aplicados")
        ...
        for field in agg_fields:
            if hasattr(Role, field):
                ...
                error_msg = f"Campo '{field}' no existe en modelo Role"
        ...
        _log_commit(session, f"[auth] ✅ Suma Role completada: {sum_result}")
    ...
    async def mean(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 📊 Calculando media de campos {agg_fields} de registros Role con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Media Role completada: {mean_result}")
    ...
    async def max(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔺 Calculando máximo de campos {agg_fields} de registros Role con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Máximo Role completado: {max_result}")
    ...
    async def min(
        ...
        Returns:
            ...
            - metadata: Información adicional sobre la operación
        """
        _log_tx(session, f"[auth] 🔻 Calculando mínimo de campos {agg_fields} de registros Role con filtros aplicados")
        ...
        _log_commit(session, f"[auth] ✅ Mínimo Role completado: {min_result}")
    ...
    async def agg(
        ...
        Examples:
            ...
            ```
        """
        _log_tx(session, f"[auth] 🎯 Realizando agregaciones en Role: {list(request.aggregations.keys())}")
        if verbose:
        ...
        for operation, fields in request.aggregations.items():
            ...
            for field in fields:
                try:
                    ...
                    if is_expression:
                        try:
                            column_expr = ExpressionParser.parse(expression, Role)
                            if alias:
                                field_key  = alias
                        ...
                        if hasattr(Role, expression):
                            column      = getattr(Role, expression)
                            column_type = str(column.type).upper()
                            ...
                            errors.append(f"Campo '{expression}' no existe en modelo Role")
        ...
        if request.group_by:
            for gb_item in request.group_by:
                ...
                if not hasattr(Role, field_name):
                    errors.append(f"Campo de group_by '{field_name}' no existe en modelo Role")
                    continue

                column      = getattr(Role, field_name)
                column_type = str(column.type).upper()
        ...
        _log_commit(session, f"[auth] ✅ Agregaciones Role completadas: {len(agg_expressions)} expresiones, {len(agg_rows)} filas")
